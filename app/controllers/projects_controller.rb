...
class ProjectsController < ApplicationController
  ...
  def index
    index! do |format|
      format.html do
        ...
        #end
        @curated_pages = CuratedPage.visible.order("created_at desc").limit(18)
        @last_tweets = Rails.cache.fetch('last_tweets', :expires_in => 30.minutes) do
          begin
            JSON.parse(Net::HTTP.get(URI("http://api.twitter.com/1/statuses/user_timeline.json?screen_name=#{t('site.twitter')}")))[0..5]
          rescue
            []
          end
        end
  ...
  def bitly
    ...
    require 'net/http'
    res = Net::HTTP.start("api.bit.ly", 80) { |http| http.get("/v3/shorten?login=crowdplaces&apiKey=R_1170aa6e19d4dfb49adbd7a80aba2704&longUrl=#{CGI.escape(project_url(@project))}") }
